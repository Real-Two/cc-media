...
function PerkIcon({ Icon, color }) {
  return (
    ...
    >
      <Icon
        size={20}
        className={color === 'accent' ? 'text-accent' : 'text-purple-light'}
      />
    </motion.div>
  )
}
    ...
    <section className="relative overflow-hidden py-8 md:py-0">
      <div className="max-w-[1400px] mx-auto px-6 md:px-12 py-24 w-full">
        ...
        >
          ...
          <h2
            className="font-heading font-bold text-text mb-20 leading-[1.1] tracking-[-0.02em]"
        ...
        >
          ...
          <motion.div
            ...
            className="animated-border bg-bg-card backdrop-blur-xl border border-border rounded-3xl p-10 md:p-14 relative overflow-hidden group hover:border-accent/30 transition-all duration-500"
          >
            ...
            <div className="relative z-10 pl-4 md:pl-6">
              <h3 className="font-heading text-4xl md:text-5xl font-bold text-text mb-4 tracking-[-0.02em]">
                Creators
              </h3>
              <p className="font-body text-text-muted text-[17px] mb-12 font-light max-w-[300px]">
            ...
            <div className="relative z-10 space-y-5">
              {creatorPerks.map(({ icon: Icon, text }) => (
                <motion.div
                  ...
                  whileHover={{ x: 6 }}
                  transition={{ type: 'spring', stiffness: 400 }}
                  className="flex items-center gap-5 bg-bg-elevated/60 p-3.5 rounded-2xl border border-border group-hover:border-border-hover transition-all duration-300"
                >
                  <PerkIcon Icon={Icon} color="accent" />
                  <span className="font-body text-[15px] text-text font-medium">
                    {text}
                  </span>
                </motion.div>
              ))}
          ...
          <motion.div
            ...
            whileHover={{ y: -6 }}
            transition={{ type: 'spring', stiffness: 300 }}
            className="animated-border bg-bg-card backdrop-blur-xl border border-border rounded-3xl p-10 md:p-14 relative overflow-hidden group hover:border-purple/30 transition-all duration-500"
          >
            ...
            <div className="relative z-10 space-y-5">
              {brandPerks.map(({ icon: Icon, text }) => (
                <motion.div
                  ...
                  className="flex items-center gap-5 bg-bg-elevated/60 p-3.5 rounded-2xl border border-border group-hover:border-border-hover transition-all duration-300"
                >
                  <PerkIcon Icon={Icon} color="purple" />
                  <span className="font-body text-[15px] text-text font-medium">
                    {text}
                  </span>
                </motion.div>
